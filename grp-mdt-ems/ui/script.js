    ...
    methods: {
        changePage(page) {
            ...
            if (page == "Home") {
                ...
                $.post('http://grp-mdt-ems/getWarrants', JSON.stringify({}));
        ...
        closeMDT() {
          ...
          $("#login input[type='password']").val('');
            $.post('http://grp-mdt-ems/close', JSON.stringify({}));
        },
        OffenderSearch() {
            if (this.offender_search) {

                this.offender_results.query = this.offender_search;
                $.post('http://grp-mdt-ems/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://grp-mdt-ems/viewOffender', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://grp-mdt-ems/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post('http://grp-mdt-ems/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://grp-mdt-ems/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        DeleteSelectedReport() {
            $.post('http://grp-mdt-ems/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://grp-mdt-ems/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://grp-mdt-ems/vehicleSearch', JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post('http://grp-mdt-ems/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://grp-mdt-ems/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://grp-mdt-ems/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://grp-mdt-ems/getReport', JSON.stringify({
                id: id
            }));
            return;
        },
        ...
        SentencePlayer() {
            ...
            $.post('http://grp-mdt-ems/sentencePlayer', JSON.stringify({
    ...
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                mdtApp.changePage("Search Patients");
...
function ClearActiveNavItems() {
    ...
    $("#search-patients").removeClass("nav-active");
...
function login(){
  ...
  $("#login input[type='password']").val()=="pillbox"){
    ...
    $("#login p").css("color", "red").html("Dados incorretos!")
    setTimeout(function(){
      $("#login p").html("")
    },3500)
  }
